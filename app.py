...
app = FastAPI(title="AI Workforce OS", version="0.2.0")
...
def model(system,prompt):
    ...
    try:
        ...
        return OpenAI(api_key=key).responses.create(
...
Create 3-6 useful tasks; the final task must be Executive Report Writer.""",
    ...
    return {"tasks":[
      {"title":"Research the objective","instructions":"Identify relevant facts, evidence needs, assumptions, unknowns and verification questions.","role":"Research Specialist"},
      {"title":"Analyze technical and operational risks","instructions":"Analyze constraints, dependencies, technical risks and alternatives using the objective and research context.","role":"Engineering Analyst"},
      {"title":"Analyze sourcing and execution risks","instructions":"Identify supply, sourcing, commercial and execution risks and mitigations.","role":"Procurement Analyst"},
      {"title":"Prepare the final executive report","instructions":"Synthesize specialist findings into conclusions, risks, uncertainties and recommended next actions.","role":"Executive Report Writer"}]}
...
def specialist(a,item,context):
    ...
    return f"""DEMO EXECUTION — {a['name']}

Task completed: {item['title']}

Summary:
Structured analysis completed using the objective and prior workforce context.

Key findings:
- Major dependencies and risks were identified.
- This demonstrates the orchestration pipeline.
- External facts remain unverified until live tools are connected.

Evidence / assumptions:
- Demo mode is active.
- No live web, supplier, or document tools are connected.

Uncertainties:
- External claims require verification.

Recommended next action:
Pass this result to the next specialist and include it in the executive report.""",.55

def execute_goal(gid):
    g=fetch("SELECT * FROM goals WHERE id=?",(gid,))[0]
    ceo=agent("CEO / Orchestrator")
    write("UPDATE goals SET status=? WHERE id=?",("planning",gid)); write("UPDATE agents SET status=? WHERE id=?",("working",ceo["id"]))
    ...
    write("UPDATE agents SET status=? WHERE id=?",("idle",ceo["id"]))
...
def home():
    ...
    goals="".join(f"<div class='row'><a href='/goals/{g['id']}'><b>{esc(g['title'])}</b></a><div class='muted'>{esc(g['status'])} · ${g['budget']:.2f} budget</div></div>" for g in gs) or "<p class='muted'>No objectives yet.</p>"
    ...
    return f"""<!doctype html><meta name=viewport content='width=device-width,initial-scale=1'><title>AI Workforce OS v0.2</title>
...
<header><b>AI WORKFORCE OS</b> · v0.2</header><main><div class=grid>
...
def create_goal(title:str=Form(...),description:str=Form(...),criteria:str=Form(...),budget:float=Form(...)):
    gid=uid(); write("INSERT INTO goals VALUES(?,?,?,?,?,?,?,?,?)",(gid,title,description,criteria,budget,"queued",None,None,now())); log(gid,"goal_created",f"Human assigned objective: {title}"); execute_goal(gid)
