...
APP_VERSION='0.4.0'; SCHEMA_VERSION='040-1'; DB=os.getenv('WORKFORCE_DB','workforce_v04.db')
...
def schedule(rid):
 while True:
  ...
  for t in ts:
   ...
   elif ready:x('UPDATE tasks SET status="ready",updated_at=? WHERE id=?',(now(),t['id']));task_pool.submit(task_run,rid,t['id']);changed=True
...
def auth(request):
 if AUTH:
  t=request.headers.get('authorization','');t=t[7:] if t.startswith('Bearer ') else t
