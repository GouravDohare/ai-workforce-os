...
# v0.4.22: bound the final synthesis context independently from worker/evaluator
# context. The final reporter only needs a compact decision record; passing every
# task artifact, claim, and provenance blob can exceed the model request ceiling.
...
def report_call(rid, prompt, spend_cap):
    """Generate the final report inside a bounded request envelope.

    v0.4.22 treats final-report context size as a first-class reliability limit.
    The caller already supplies a compact evidence/task record; this function also
    protects the request itself and retries once with an even smaller prompt if the
    provider rejects/truncates the first synthesis.
    """
    prompt = clip(prompt, FINAL_REPORT_CONTEXT_CHARS)
    try:
        return call(rid,None,"report",prompt,MODEL,False,None,FINAL_REPORT_OUTPUT_TOKENS,spend_cap)
    except Exception as e:
        msg=str(e)
        if not any(x in msg for x in (
            "incomplete_output:max_output_tokens",
            "strategy_error:model request too large",
        )):
            raise
        event(rid,"REPORT_ESCALATED","final report request needed a compact recovery",
              {"initial_context_chars":len(prompt),"fallback_context_chars":FINAL_REPORT_FALLBACK_CHARS,
               "initial_tokens":FINAL_REPORT_OUTPUT_TOKENS,"fallback_tokens":FINAL_REPORT_FALLBACK_OUTPUT_TOKENS,
               "reason":msg[:300]})
        print(f"REPORT_ESCALATED run={rid} context={len(prompt)} fallback={FINAL_REPORT_FALLBACK_CHARS}",flush=True)
        compact=(
            "Write a concise but complete decision-ready final report. Address EVERY success criterion. "
            "Use headings and bullets where useful. Preserve important evidence, contradictions, "
            "assumptions, unknowns and validation needs. Never invent facts or claim real-world actions "
            "occurred without tool evidence. The workforce execution budget is AI/API spend only; "
            "do not present it as the business launch budget.\n" + clip(prompt, FINAL_REPORT_FALLBACK_CHARS)
        )
...
def final_report_context(rid, g, ts, latest_eval):
    """Build a bounded, decision-focused record for the final reporter.

    Keep enough provenance to support important claims, but do not forward full
    worker JSON, long source metadata, or every historical claim. Contradictions
    and budget items are explicitly preserved rather than silently collapsed.
    """
    tasks=[]
    for t in ts:
        tasks.append({
            "title":clip(t["title"],180),
            "output":clip(t["output"],FINAL_TASK_OUTPUT_CHARS),
            "structured":clip(jd(jl(t["structured"])),FINAL_TASK_STRUCTURED_CHARS),
        })
    evidence=[]
    for e in q("SELECT id,title,url,publisher,published_at,metadata FROM evidence WHERE run_id=? ORDER BY retrieved_at DESC LIMIT ?",(rid,FINAL_EVIDENCE_ITEMS)):
        evidence.append({
            "id":e["id"],"title":clip(e["title"],160),"url":clip(e["url"],400),
            "publisher":clip(e["publisher"],100),"published_at":e["published_at"],
        })
    claims=[]
    for c in q("SELECT id,task_id,claim,claim_type,values_json,source_ids,confidence,status FROM claims WHERE run_id=? ORDER BY created_at DESC LIMIT ?",(rid,FINAL_CLAIM_ITEMS)):
        claims.append({
            "id":c["id"],"task_id":c["task_id"],"claim":clip(c["claim"],FINAL_CLAIM_CHARS),
            "claim_type":c["claim_type"],"values":clip(c["values_json"],700),
            "source_ids":clip(c["source_ids"],500),"confidence":c["confidence"],"status":c["status"],
        })
    conflicts=[]
    for c in contradiction_summary(rid)[:FINAL_CONTRADICTION_ITEMS]:
        conflicts.append({
            "severity":c.get("severity"),
            "claim_a":clip(c.get("claim_a"),FINAL_CONTRADICTION_CHARS),
            "claim_b":clip(c.get("claim_b"),FINAL_CONTRADICTION_CHARS),
            "reason":clip(c.get("reason"),FINAL_CONTRADICTION_REASON_CHARS),
            "status":c.get("status"),
        })
    budgets=[]
    for b in q("SELECT id,task_id,label,amount,currency,period,basis,source_ids,status FROM budget_items WHERE run_id=? ORDER BY created_at LIMIT ?",(rid,FINAL_BUDGET_ITEMS)):
        budgets.append({
            "task_id":b["task_id"],"label":clip(b["label"],180),"amount":b["amount"],
            "currency":b["currency"],"period":clip(b["period"],80),"basis":clip(b["basis"],180),
            "source_ids":clip(b["source_ids"],400),"status":b["status"],
        })
    eval_summary={}
    if latest_eval:
        eval_summary={
            "score":latest_eval["score"],"passed":latest_eval["passed"],
            "dimensions":clip(latest_eval["dimensions"],1200),
            "failures":clip(latest_eval["failures"],FINAL_EVAL_CHARS),
            "recommendations":clip(latest_eval["recommendations"],1800),
            "contradictions":clip(latest_eval["contradictions"],1800),
        }
    return {
        "objective":clip(g["title"],500),
        "success_criteria":clip(g["criteria"],5000),
        "workforce_execution_budget_usd":float(g["budget"]),
        "verified_tasks":tasks,
        "evidence":evidence,
        "claims":claims,
        "contradictions":conflicts,
        "business_budget_items":budgets,
        "verification_summary":eval_summary,
    }
...
def execute(rid):
    ...
    try:
        ...
        final_ctx=final_report_context(rid,g,ts,latest_eval)
...
COMPACT VERIFIED RECORD: {jd(final_ctx)}
Never invent facts or source details. For every important numerical/current factual claim, cite the supporting source from the compact evidence/claim registry when one exists and distinguish verified facts, estimates, assumptions, recommendations, unknowns, and unresolved contradictions. If sources disagree, explain the scope/date/definition difference or explicitly preserve the conflict; never silently choose one number. Treat the workforce execution budget as unrelated to the business budget. Do not turn proposed validation steps into claims that validation already occurred. If the record is incomplete, say what is missing instead of fabricating it."""
