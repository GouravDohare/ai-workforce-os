...
def recover_after_restart():
 # The executor is process-local. Any active run from a previous process cannot resume safely.
 # Mark it interrupted so the UI is honest and the user can retry instead of seeing a permanent planning state.
 with lock:
  c=db(); c.execute("UPDATE runs SET status='interrupted',reason='process restarted before run completed',ended_at=? WHERE status IN ('queued','planning','executing','evaluating','replanning')",(now(),)); c.execute("UPDATE goals SET status='interrupted',verification_status='interrupted',updated_at=? WHERE run_id IN (SELECT id FROM runs WHERE status='interrupted') AND status IN ('queued','planning','executing','evaluating','replanning')",(now(),)); c.execute("UPDATE tasks SET status='interrupted',error_type='transient',error_message='process restarted before task completed',updated_at=? WHERE run_id IN (SELECT id FROM runs WHERE status='interrupted') AND status IN ('pending','waiting_dependency','ready','running','retrying')",(now(),)); c.commit(); c.close()
...
if __name__=='__main__':
 import uvicorn;init();recover_after_restart();uvicorn.run(app,host='0.0.0.0',port=int(os.getenv('PORT','10000')))
